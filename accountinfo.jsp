...
<body >
	<div class="search-title">
		<h2>View current Account Information</h2>
		<div class="description"></div>
	</div>
	<table border="0" cellspacing="1" class="grid" cellpadding="0"
		width="100%">
		<tr>
			<td align="right" width="10%" class="title">account name:</td>
			<td>${requestScope.users.username}</td>
			<td colspan="2" rowspan="5"><img src="${requestScope.users.xiangpian}" width="200"
				height="200" />
			</td>
		</tr>
	
		<tr>
			<td align="right" class="title">create time:</td>
			<td>${requestScope.users.createtime}</td>
		</tr>
		<tr>
			<td align="right" width="10%" class="title">email :</td>
			<td>${requestScope.users.email}</td>
		</tr>
		
		<tr>
			<td align="right" width="10%" class="title">login times:</td>
			<td>${requestScope.users.logtimes}</td>
		</tr>
		<tr>
			<td align="right" width="10%" class="title">nick name:</td>
			<td>${requestScope.users.nickname}</td>
		</tr>
		<tr>
			<td align="right" width="10%" class="title">name:</td>
			<td>${requestScope.users.realname}</td>
		
			<td align="right" class="title">sex :</td>
			<td>${requestScope.users.sex}</td>
		</tr>
	
		<tr>
			<td align="right" width="10%" class="title">mobile:</td>
