...
<h2>
				    ...
				    <form name="hytypeform"  method="post" action="<%=SystemParam.getSiteRoot()%>/admin/hytypemanager.do"  id="hytypeForm">
				        <table class="grid" cellspacing="1" width="100%">
						        <input type="hidden" name="id" value="${id}" />
						        <input type="hidden" name="actiontype" value="${actiontype}" />
						         <input type="hidden" name="seedid" value="${seedid}" />
						         <input type="hidden" name="errorurl" value="/admin/hytypeadd.jsp" />
						        <input type="hidden" name="forwardurl" value="/admin/hytypemanager.do?actiontype=get&forwardurl=/admin/hytypemanager.jsp" />
						        <tr>
								  <td colspan="4">
								      ${errormsg}
								  </td>
											   ...
											   <td>
												   <input name="name" placeholder="名称" validate="{required:true,messages:{required:'请输入名称'}}" value="${requestScope.hytype.name}" class="input-txt" type="text" id="txtName"  />
												</td>
											   </tr>
						   <tr>
						       <td colspan="4">
