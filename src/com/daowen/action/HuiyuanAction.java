...
public class HuiyuanAction extends PageActionBase {
	...
	private void exit() {
		...
		if (request.getSession().getAttribute("researcher") != null) {
			...
			System.out.println("专家退出");
			request.getSession().removeAttribute("researcher");

		}
		if (request.getSession().getAttribute("papereditor") != null) {

			System.out.println("编辑人员退出");
			request.getSession().removeAttribute("papereditor");
	...
	private void huiyuanLogin() {
		...
		if (huiyuan != null && huiyuan.getPassword().equals(password)) {
			...
			dispatchParams(request, response);
			request.setAttribute("message", "<label class='error'>account password not match</label>");
			try {
